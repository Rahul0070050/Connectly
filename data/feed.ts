const profile =
  "data:image/jpeg;base64,";
import redfox from "../public/feed/red-fox.jpeg";
import Redfox from "../public/feed/Red-fox.webp";
import butterflies from "../public/feed/butterflies-flowers.avif";
import flowers from "../public/feed/flowers.jpg";
import sunsetsky from "../public/feed/sunset-sky.jpg";
import temple from "../public/feed/temple.jpg";
export const feeds = [
  {
    avatar: profile,
    userName: "Rahul OR",
    feed: redfox,
    caption: "Red fox is a fox that.",
    likes: 100,
    comments: [
      {
        user: profile,
        name: "Rahul OR",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
    ],
    time: "2023-08-01T12:00:00Z",
    tags: ["fox", "red", "animal"],
  },
  {
    avatar: profile,
    userName: "Rahul OR",
    feed: Redfox,
    caption: "Red fox is a fox that is predominantly red in color. Red",
    likes: 100,
    comments: [
      {
        user: profile,
        name: "Rahul OR",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
    ],
    time: "2023-08-01T12:00:00Z",
    tags: ["fox", "red", "animal"],
  },
  {
    avatar: profile,
    userName: "Rahul OR",
    feed: butterflies,
    caption:
      "flowers are beautiful and colorful plants that come in a variety of shapes and sizes. They are often used in floral arrangements and are known for their sweet fragrance.",
    likes: 100,
    comments: [
      {
        user: profile,
        name: "Rahul OR",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
    ],
    time: "2023-08-01T12:00:00Z",
    tags: ["flowers", "plants"],
  },
  {
    avatar: profile,
    userName: "Rahul OR",
    feed: flowers,
    caption:
      "flowers are beautiful and colorful plants that come in a variety of shapes and sizes. They are often used in floral arrangements and are known for their sweet fragrance.",
    likes: 100,
    comments: [
      {
        user: profile,
        name: "Rahul OR",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
    ],
    time: "2023-08-01T12:00:00Z",
    tags: ["flowers", "plants"],
  },
  {
    avatar: profile,
    userName: "Rahul OR",
    feed: sunsetsky,
    caption:
      "sunset is the time of day when the sun disappears below the horizon. It is often associated with a beautiful and peaceful atmosphere.",
    likes: 100,
    comments: [
      {
        user: profile,
        name: "Rahul OR",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
    ],
    time: "2023-08-01T12:00:00Z",
    tags: ["sunset", "sun", "atmosphere"],
  },
  {
    avatar: profile,
    userName: "Rahul OR",
    feed: temple,
    caption:
      "temple is a place of worship for a particular religion. It is often associated with spirituality and religious practices.",
    likes: 100,
    comments: [
      {
        user: profile,
        name: "Rahul OR",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
      {
        user: profile,
        name: "Aloshy",
        comment: "This is a comment",
      },
    ],
    time: "2023-08-01T12:00:00Z",
    tags: ["temple", "worship", "spirituality"],
  },
];
